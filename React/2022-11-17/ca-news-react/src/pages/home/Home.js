import './Home.css';

const Home = () => {

    return (
        <>
            <header>
                <div className="child-three">
                    <h1>Programuok savo ateitį!&#174;</h1>
                    <h2>CodeAcademy programavimo kursai – tiesiausias kelias technologinės karjeros link</h2>
                </div>
                <div className="child-four">
                    <div className="studentams">
                        <h2>Studentams</h2>
                        <p>Ar žengi pirmą ar kažkelintą žingsnį link tech išsilavinimo – mes tau padėsim!</p>
                        <p>Tiesiausias kelias link karjeros – per mus. 🏁 CodeAcademy programavimo kursai</p>
                        <a href="./#" type="button" className="button">sužinok&nbsp;daugiau</a>
                    </div>
                    <div className="verslui">
                        <h2>Verslui</h2>
                        <p>Esame geriausias kompanijų draugas Edutech erdvėje.</p>
                        <p>Kelsime Jūsų darbuotojų kompetenciją bei atrasime talentus. 📈CodeAcademy programavimo kursai</p>
                        <a href="./#" type="button" className="button">sužinok&nbsp;daugiau</a>
                    </div>
                </div>
            </header>
            <main>
                <div className="child-one">
                    <div className="section-1-1">
                        <h1>CodeAcademy pulsas</h1>
                    </div>
                    <div className="section-1-2">
                        <p>Manome, jog technologinis išsilavinimas privalo sekti šiandienos ir rytojaus darbo rinkos tendencijas
                            bei būti prieinamas kiekvienam. 🙌</p>
                        <p>Šiuo principu vadovaudamiesi, kas dieną savo studentams atveriame naujas galimybes. 🔝</p>
                    </div>
                    <div className="section-1-3">
                        <div className="section-3-1">
                            <h1>398</h1>
                            <p>Šiuo metu mokosi studentų</p>
                        </div>
                        <div className="section-3-2">
                            <h1>1359</h1>
                            <p>2021 m. baigė studentų</p>
                        </div>
                        <div className="section-3-3">
                            <h1>80%</h1>
                            <p>Baigę Pažengusių studijas įsidarbino</p>
                        </div>
                        <div className="section-3-4">
                            <h1> Front-End</h1>
                            <p>Populiariausia programa</p>
                        </div>
                    </div>
                </div>
                <div className="child-two">
                    <div className="section-2-1">
                        <h1>Jei galiu aš - gali ir tu!</h1>
                        <p>Kas sieja buvusį pareigūną, o dabar pensininką Kęstutį, operos solistą Algirdą ir buvusią statybų
                            inžinierę Rugilę? 🤔</p>
                        <a href="./#" type="button" className="button">išgirsk&nbsp;istorijas!</a>
                    </div>
                    <div className="section-2-2">
                    </div>
                </div>
                <div className="child-three">
                    <div className="section-3-1">
                        <iframe width="770" height="434" src="https://www.youtube.com/embed/b8dGCsP75HA"
                            title="Susipažink su herojais! | Jei galiu aš - gali ir tu!" frameBorder="0"
                            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                            allowFullScreen></iframe>
                    </div>
                </div>
                <div className="child-four">
                    <div className="section-4-1">
                        <h1>Atrask skirtumus</h1>
                    </div>
                    <div className="section-4-2">
                        <h2>Klaidingų pasirinkimų čia nėra – jei mąstai apie programuotojo karjerą, esi teisingame kelyje
                            💪</h2>
                    </div>
                    <div className="section-4-3">
                        <table>
                            <tbody>
                                <tr>
                                    <td></td>
                                    <td>CodeAcademy</td>
                                    <td>Universitetai</td>
                                    <td>Kitos programavimo akademijos</td>
                                    <td>Online kursai</td>
                                </tr>
                                <tr>
                                    <td>Trukmė</td>
                                    <td>nuo 2 iki 9 mėn.</td>
                                    <td>3-4 metai</td>
                                    <td>nuo 1 iki 3 mėn.</td>
                                    <td>3-18 mėn.</td>
                                </tr>
                                <tr>
                                    <td>Mokymosi valandos</td>
                                    <td>nuo 160 iki 1060 val.</td>
                                    <td>1600 val.</td>
                                    <td>nuo 90 iki 450 val.</td>
                                    <td>nuo 20 iki 80 val.</td>
                                </tr>
                                <tr>
                                    <td>Darbovietės paieška</td>
                                    <td>✓</td>
                                    <td>✕</td>
                                    <td>✓</td>
                                    <td>✕</td>
                                </tr>
                                <tr>
                                    <td>Karjeros planavimas</td>
                                    <td>✓</td>
                                    <td>✓</td>
                                    <td>✓</td>
                                    <td>✕</td>
                                </tr>
                                <tr>
                                    <td>Motyvacijos išlaikymo skatinimas</td>
                                    <td>✓</td>
                                    <td>✓</td>
                                    <td>✓</td>
                                    <td>✕</td>
                                </tr>
                                <tr>
                                    <td>Bendruomenė ir renginiai</td>
                                    <td>✓</td>
                                    <td>✓</td>
                                    <td>✓</td>
                                    <td>✕</td>
                                </tr>
                                <tr>
                                    <td>Mokymasis nuotoliniu būdu</td>
                                    <td>✓</td>
                                    <td>✓</td>
                                    <td>✓</td>
                                    <td>✓</td>
                                </tr>
                                <tr>
                                    <td>Lankstus išsimokėjimas</td>
                                    <td>✓</td>
                                    <td>✕</td>
                                    <td>✓</td>
                                    <td>✕</td>
                                </tr>
                                <tr>
                                    <td>Mokėjimas tik po studijų baigimo</td>
                                    <td>✓</td>
                                    <td>✕</td>
                                    <td>✕</td>
                                    <td>✕</td>
                                </tr>
                                <tr>
                                    <td>Dėstytojai iš top IT kompanijų</td>
                                    <td>✓</td>
                                    <td>✓</td>
                                    <td>✓</td>
                                    <td>✕</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
                <div className="child-five">
                    <div className="section-5-1"></div>
                    <div className="section-5-2"></div>
                    <div className="section-5-3"></div>
                </div>
            </main>
        
        </>
    );
}

export default Home;